            ...
            if (isOverNetwork) {
                ...
                if (enemyStatus.contains(ERROR_FEEDBACK)) {
                    ...
                    try {
                        ...
                        System.out.println("The opponent has sent an erroneous output!");
                    }
                    players.get(1).setExited(opponentWon);
                    players.get(1).setMovesMade(opponentMoves);

                    if (players.get(0).isGameOver() && !opponentWon) {
                        System.out.println("You won the race!");
                    } else if (opponentWon && !players.get(0).isGameOver()) {
